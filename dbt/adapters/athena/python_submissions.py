    ...
    def submit(self, compiled_code: str) -> Any:
        ...
        if compiled_code.strip() and self.application_ready:
            ...
            try:
                ...
                raise DbtRuntimeError(f"""Unable to start EMR job
dbt invocation id:   {self.invocation_id}
dbt model:           {self.relation_name}
...
""")
            ...
            LOGGER.debug(f"""EMR job started:
dbt invocation id:   {self.invocation_id}
dbt model:           {self.relation_name}
emr application id:  {self.application_id}
...
script location:        {script_location}
error message:          {err}
Stdout s3 path:         3://{self.s3_bucket}/{self.s3_log_prefix}/applications/{self.application_id}/jobs/{job_run_id}/SPARK_DRIVER/stdout.gz
"""
                    )
